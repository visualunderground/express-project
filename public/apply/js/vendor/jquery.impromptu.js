 ...
 * Last Modified By: Philip Vellender
 *
 * This file has been modified from the standard plugin
 * to add a new 'buttonText' property. This can be used
 * as follows to pass descriptive strings:
 buttons:{Yes:true,No:false},
 buttonText: {Yes:'Stay logged in',No:'Log out'}
 *
 */
...
jQuery.extend({
    ...
    ImpromptuStateDefaults: {
        html: '', buttons: {Ok: true}, buttonText: {}, focus: 0, submit: function () {
            return true;
        }
    },
    ...
    ImpromptuClose: function () {
        jQuery('#' + jQuery.ImpromptuCurrentPrefix + 'box').fadeOut('fast', function () {
            jQuery(this).remove();
        });
    },
    ...
    prompt: function (m, o) {
        ...
        msgbox += '<div class="' + o.prefix + '" id="' + o.prefix + '"><div class="' + o.prefix + 'container"><div id="' + o.prefix + 'close" class="' + o.prefix + 'close">Close</div><div id="' + o.prefix + 'states"></div>';
        msgbox += '</div></div></div>';
        ...
        if (m.constructor == String) {
            m = {state0: {html: m, buttons: o.buttons, buttonText: o.buttonText, focus: o.focus, submit: o.submit}};
        }
        ...
        jQuery.each(m, function (statename, stateobj) {
            ...
            jQuery.each(stateobj.buttons, function (k, v) {
                if (typeof o.buttonText[k] !== 'undefined') {
                    var bt = o.buttonText[k];
                } else {
                    bt = k;
                }
                states += '<button name="' + o.prefix + '_' + statename + '_button' + k + '" id="' + o.prefix + '_' + statename + '_button' + k + '" rel="' + k + '" value="' + v + '">' + bt + '</button>';
            });
            states += '</div></div>';
        });
        ...
        jQuery.each(m, function (statename, stateobj) {
            ...
            state.children('.' + o.prefix + 'buttons').children('button').click(function () {
                ...
                if (typeof o.buttonText[jQuery(this).attr('rel')] !== 'undefined') {
                    var k = jQuery(this).attr('rel');
                } else {
                    k = jQuery(this).text();
                }
                var clicked = stateobj.buttons[k];
                var forminputs = {};
